...
def upload_files_to_s3():
    ...
    print(f"Uploading Reference Data to S3 bucket: {bucket_name}")
    s3_client.put_object(Body=reference_data, Bucket=bucket_name, Key=reference_key, ContentType="text/csv")
    print(f"Reference Data uploaded to S3 bucket: {bucket_name} with key: {reference_key}")

    print(f"Uploading CSV File to S3 bucket: {bucket_name}")
    s3_client.put_object(Body=csv_file, Bucket=bucket_name, Key=csv_key, ContentType="text/csv")
    print(f"CSV File uploaded to S3 bucket: {bucket_name} with key: {csv_key}")

    print(f"Uploading JSON File to S3 bucket: {bucket_name}")
    s3_client.put_object(Body=json_file, Bucket=bucket_name, Key=json_key, ContentType="application/json")
    print(f"JSON File uploaded to S3 bucket: {bucket_name} with key: {json_key}")
